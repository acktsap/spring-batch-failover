                ...
                chunk<Customer, TargetCustomer>(3, transactionManager) {
                    ...
                    writer { chunk ->
                        for (targetCustomer in chunk) {
                            if (targetCustomer.customerId == 8L) {
                    ...
                    faultTolerant {
                        ...
                        skipLimit(3)
                        listener(
                            object : SkipListener<Customer, TargetCustomer> {
                                override fun onSkipInWrite(item: TargetCustomer, t: Throwable) {
                                    logger.info(t) { "Error occurred in writer (item: $item)" }
                                }
                            }
                        )
                    }
