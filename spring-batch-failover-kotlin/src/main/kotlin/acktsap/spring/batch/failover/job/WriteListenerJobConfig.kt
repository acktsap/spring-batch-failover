...
class WriteListenerJobConfig(
    ...
    @Bean
    fun writeListenerJob(): Job = batch {
        job("writeListenerJob") {
            step("insertToTargetStep") {
                chunk<Customer, TargetCustomer>(3, transactionManager) {
                    ...
                    listener(
                        object : ItemWriteListener<TargetCustomer> {
                            override fun onWriteError(exception: Exception, items: Chunk<out TargetCustomer>) {
